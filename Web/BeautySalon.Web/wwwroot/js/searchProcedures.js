        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                data.procedures.forEach(procedure => {
                    result += '<div class="col-md-4">' + '<div class="block-7">' + '<div class="text-center">' + `<h3 class="heading-2 my-4 heading-color">${procedure.name}</h3>` + `<span class="price ">€${procedure.formattedPrice}</span>` + '<ul class="pricing-text mb-5">' + `<li><strong>Category:</strong> ${procedure.categoryName}</li>` + ` <li><strong>Skin Type:</strong> ${procedure.skinTypeToDisplay}</li>` + `<li><strong>Raiting: </strong>${procedure.formattedRaiting}</li>` + '</ul>' + `<a class="btn btn-primary d-block px-2 py-4 custom-hover" href="/Procedures/GetDetails/${procedure.id}"">Details</a>` + '</div>' + '</div>' + '</div>';
                });
